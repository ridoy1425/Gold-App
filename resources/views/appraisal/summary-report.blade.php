...
@section('content_title')
    <h4 class="mt-2">Appraisal Summary Report</h4>
    ...
    <div class="row page-content">
        <div class="container">
            ...
            <div class="card card-default">
                <div class="m-3">
                    <h6>Summary Search</h6>
                    <form method="GET" action="{{ url('appraisal/summary-report') }}" id="report_form" autocomplete="off">
                ...
                @if (isset($branch_id) && isset($category_id))
                    @php
                        $branchCategory = App\Models\BranchCategory::where('branch_id', $branch_id)
                            ->where('category_id', $category_id)
                            ->with([
                                'DutyResponsibility' => function ($q) {
                                    $q->select('branch_category_id', \DB::raw('sum(marks) as total_marks'))->groupBy('branch_category_id');
                                },
                                'AttitudeBehavior' => function ($q) {
                                    $q->select('branch_category_id', \DB::raw('sum(marks) as total_marks'))->groupBy('branch_category_id');
                                },
                                'hrAdministrative' => function ($q) {
                                    $q->select('branch_category_id', \DB::raw('sum(marks) as total_marks'))->groupBy('branch_category_id');
                                },
                                'finance' => function ($q) {
                                    $q->select('branch_category_id', \DB::raw('sum(marks) as total_marks'))->groupBy('branch_category_id');
                            ...
                            ->first();
                        if ($branchCategory) {
                            $totalMarksDutyResponsibility = $branchCategory->DutyResponsibility->sum('total_marks') ?? 0;
                            $AttitudeBehavior = $branchCategory->AttitudeBehavior->sum('total_marks') ?? 0;
                            $Hr = $branchCategory->hrAdministrative->sum('total_marks') ?? 0;
                            $finance = $branchCategory->finance->sum('total_marks') ?? 0;
                            $totalMarks3 = $AttitudeBehavior + $Hr + $finance;
                        } else {
                            $totalMarksDutyResponsibility = 0;
                            $totalMarks3 = 0;
                        }

                        $totalLeave = App\Models\LeaveType::sum('no_of_days') ?? 0;
                    @endphp
                @endif
                <div class="card-body">
                    <table class="cell-border" id="table_id">
                        <thead>
                            <tr>
                                ...
                                <th style="text-align:center;">Duty & Responsiblity
                                    <br>({{ $totalMarksDutyResponsibility ?? 0 }})
                                </th>
                                <th style="text-align:center;">Attitude & Behavior <br> ({{ $totalMarks3 ?? 0 }})
                                </th>
                                <th style="text-align:center;">Leave Balance<br> ({{ $totalLeave ?? 0 }})</th>
                            </tr>
                        </thead>
                        <tbody>
                            @if (isset($employees))
                                ...
                                @foreach ($employees as $employee)
                                    @php
                                        $appraisal = App\Models\Appraisal::where('employee_id', $employee->id)
                                            ->whereRaw('YEAR(evaluation_date) = ?', [$year])
                                            ->first();
                                        $totalDutyMarks = $appraisal->duty_responsibility()->sum('mark') ?? 0;
                                        $totalAttitudeMarks = $appraisal->attitude_behavior()->sum('mark') ?? 0;
                                        $totalHrMarks = $appraisal->hr_administrative()->sum('mark') ?? 0;
                                        $totalFinanceMarks = $appraisal->finance()->sum('mark') ?? 0;

                                        $secondSectionTotal = $totalAttitudeMarks + $totalHrMarks + $totalFinanceMarks;

                                        $totalLeavesByType = $employee
                                            ->leaveEntry()
                                            ->where('status_id', 42)
                                            ->where(function ($query) use ($year) {
                                                return $query->whereYear('accept_from', '=', $year);
                                            })
                                            ->sum('no_of_days');
                                    @endphp
                                    <tr>
                                        ...
                                        <td>{{ $year ?? '' }}</td>
                                        <td>{{ $totalDutyMarks }}</td>
                                        <td>{{ $secondSectionTotal }}</td>
                                        <td>{{ $totalLeavesByType ?? 0 }}</td>
                                    </tr>
                                @endforeach
                            @endif
                        </tbody>
